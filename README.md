...
Latest metrics (generated: 2025-12-07 00:00 UTC) — values come from the most recent run of `show_model_metrics.py` and may change when you retrain or change the data:

| Model | Accuracy | F1-Score | Precision | Recall |
|-------|---------:|---------:|----------:|-------:|
| Logistic Regression | 96.00% | 96.03% | 96.14% | 96.00% |
| Random Forest       | 95.00% | 95.03% | 95.33% | 95.00% |
| SVM                 | 96.00% | 96.00% | 96.09% | 96.00% |
| XGBoost             | 95.50% | 95.52% | 95.63% | 95.50% |

The project performs automatic model selection based on evaluation metrics (F1-score by default). Results vary by dataset split and random seed — in the latest run the selected best model was **Logistic Regression**. The Streamlit UI displays the selected best model and the full per-model metrics dynamically (see **Model Performance** page).

To regenerate metrics locally (retrain and update the UI):

```bash
. .venv/bin/activate
.venv/bin/python show_model_metrics.py
``` 

This writes `model_metrics.json` (used by the app) and `hrews_model.pkl` (the saved best model). The Streamlit app will show the updated values after you restart or click the "Recompute metrics" button on the **Model Performance** page.
